...
namespace {
    std::vector<std::byte> fromStr(
        ...
        while (n > 0) {
            ...
            if (n < nCharsPerByte || std::any_of(ptr, ptr + nCharsPerByte, [](unsigned char c) {
                    return std::isspace(c);
                }))
                throw std::invalid_argument(
                        "Truncated byte: " + std::string(ptr, std::min(n, nCharsPerByte)));
            uint8_t value;
            std::from_chars_result conv = std::from_chars(ptr, ptr + nCharsPerByte, value, base);
            if (conv.ec != std::errc())
                throw std::system_error(std::make_error_code(conv.ec));
            if (conv.ptr != ptr + nCharsPerByte)
                throw std::invalid_argument(
                        "Invalid characters in byte: " + std::string(ptr, ptr + nCharsPerByte));
    ...
    std::string toStr(
            const std::byte *data, std::size_t n, std::size_t base, std::size_t nCharsPerByte) {
        if (n == 0)
            return "";
        ...
        while (n > 0) {
            ...
            std::to_chars_result result = std::to_chars(ptr, ptr + nCharsPerByte, *data_, base);
...
namespace Bytestream {
    std::vector<std::byte> fromHex(const std::string &str) { return fromStr(str, 16, 2); }
    std::vector<std::byte> fromBinary(const std::string &str) { return fromStr(str, 2, 8); }
