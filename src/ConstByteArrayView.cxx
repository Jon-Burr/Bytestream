...
namespace Bytestream {
    ...
    std::string ConstByteArrayView::toHex() const { return Bytestream::toHex(data(), size()); }

    std::string ConstByteArrayView::toBinary() const {
        return Bytestream::toBinary(data(), size());
    }

    ConstByteArrayView ConstByteArrayView::view(std::size_t idx, std::size_t nBytes) const {
        if (idx + nBytes > size())
    ...
    void ConstByteArrayView::readBitsInto(
            void *target, std::size_t bitPos, std::size_t nBits, Endian sourceEndianness,
    ...
    void ConstByteArrayView::readBytesInto(
            void *target, std::size_t bytePos, std::size_t nBytes, Endian sourceEndianness,
            Endian targetEndianness) const {
