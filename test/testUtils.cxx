...
{
    BOOST_CHECK(
        (std::is_same_v<typename Bytestream::BytesInfo<3>::uint_t, uint32_t>));
...
{
    BOOST_CHECK(
        Bytestream::reverseBits(std::byte{0b00101101}) ==
        std::byte{0b10110100});
    BOOST_CHECK(
        Bytestream::reverseBits(std::byte{0b00001111}) ==
        std::byte{0b11110000});
    ...
    BOOST_CHECK(
        Bytestream::swapEndianness(uint64_t(0x43ab65f289cd009e)) ==
        0x9e00cd89f265ab43);
}