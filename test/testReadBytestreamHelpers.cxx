...
BOOST_AUTO_TEST_CASE(readHelpers) {
    ...
    uint32_t nBytes;
    stream >> prefix >> elinkCount >> Bytestream::makeReader(commTimeStamp, 3 * CHAR_BIT) >>
            nWords >> leadingTime >> timeWidth >> Bytestream::makeReader(channel, 5) >>
            Bytestream::makeReader(tdc, 4) >> errorFlag >> elinkCount2 >> fixed >> elinkCount3 >>
            evtID >> suffix >> Bytestream::makeReader(elinkNumber, Bytestream::Endian::Little) >>
            Bytestream::makeReader(nBytes, Bytestream::Endian::Little);
