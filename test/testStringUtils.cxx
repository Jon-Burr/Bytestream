...
BOOST_AUTO_TEST_CASE(hex)
{
    ...
    std::vector<std::byte> testVal = {std::byte{0x23}, std::byte{0xaf},
                                      std::byte{0xe4}, std::byte{0x05}};
    ...
    testVal = {std::byte{0x43}, std::byte{0xa2}, std::byte{0xf3},
               std::byte{0x03}, std::byte{0x00}, std::byte{0x97},
               std::byte{0xc6}};
    ...
    BOOST_CHECK(Bytestream::toHex(testVal.data(), testVal.size()) ==
                "43 a2 f3 03 00 97 c6");
...
BOOST_AUTO_TEST_CASE(binary)
{
    ...
    std::vector<std::byte> testVal{std::byte{0b01011001},
                                   std::byte{0b11000101}};
    BOOST_CHECK(testVal == bytes);
    // Make sure having spaces is OK
    BOOST_CHECK(testVal == Bytestream::fromBinary("\t 01011001\n 11000101   "));
    // Make sure it chokes on bad format
    BOOST_CHECK_THROW(Bytestream::fromBinary("0101 1100"),
                      std::invalid_argument);
    BOOST_CHECK_THROW(Bytestream::fromBinary("12f31000"),
                      std::invalid_argument);
    // Make sure we can go back to a string
    BOOST_CHECK(Bytestream::toBinary(bytes.data(), bytes.size()) ==
                "01011001 11000101");
