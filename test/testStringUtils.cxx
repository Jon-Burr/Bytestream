...
BOOST_AUTO_TEST_CASE(hex)
{
    ...
    BOOST_CHECK(
        Bytestream::toHex(testVal.data(), testVal.size()) ==
...
BOOST_AUTO_TEST_CASE(binary)
{
    ...
    std::vector<std::byte> testVal{
        std::byte{0b01011001}, std::byte{0b11000101}};
    BOOST_CHECK(testVal == bytes);
    // Make sure having spaces is OK
    BOOST_CHECK(testVal == Bytestream::fromBinary("\t 01011001\n 11000101   "));
    // Make sure it chokes on bad format
    BOOST_CHECK_THROW(
        Bytestream::fromBinary("0101 1100"), std::invalid_argument);
    BOOST_CHECK_THROW(
        Bytestream::fromBinary("12f31000"), std::invalid_argument);
    // Make sure we can go back to a string
    BOOST_CHECK(
        Bytestream::toBinary(bytes.data(), bytes.size()) ==
        "01011001 11000101");
